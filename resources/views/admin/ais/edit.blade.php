...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ai.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ais.update", [$ai->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="landing_page_id">{{ trans('cruds.ai.fields.landing_page') }}</label>
                <select class="form-control select2 {{ $errors->has('landing_page') ? 'is-invalid' : '' }}" name="landing_page_id" id="landing_page_id">
                    @foreach($landing_pages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('landing_page_id') ? old('landing_page_id') : $ai->landing_page->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('campaign') ? 'is-invalid' : '' }}" name="campaign_id" id="campaign_id">
                    @foreach($campaigns as $id => $entry)
                        <option value="{{ $id }}" {{ (old('campaign_id') ? old('campaign_id') : $ai->campaign->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('recommendations') ? 'is-invalid' : '' }}" name="recommendations" id="recommendations">{{ old('recommendations', $ai->recommendations) }}</textarea>
