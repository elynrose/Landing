        ...
        <div class="col-md-12">
            @can('ai_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.ais.create') }}">
                            ...
                            <tbody>
                                @foreach($ais as $key => $ai)
                                    <tr data-entry-id="{{ $ai->id }}">
                                        ...
                                        <td>
                                            @can('ai_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.ais.show', $ai->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('ai_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.ais.edit', $ai->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('ai_delete')
                                                <form action="{{ route('frontend.ais.destroy', $ai->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('ai_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.ais.massDestroy') }}",
