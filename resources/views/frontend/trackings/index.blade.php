        ...
        <div class="col-md-12">
            @can('tracking_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.trackings.create') }}">
                        ...
                        <table class=" table table-bordered table-striped table-hover datatable datatable-Tracking">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        {{ trans('cruds.tracking.fields.created_at') }}
                                    </th>
                                    <th>
                                        &nbsp;
                                    </th>
                            ...
                            <tbody>
                                @foreach($trackings as $key => $tracking)
                                    <tr data-entry-id="{{ $tracking->id }}">
                                        ...
                                        <td>
                                            @can('tracking_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.trackings.show', $tracking->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('tracking_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.trackings.edit', $tracking->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('tracking_delete')
                                                <form action="{{ route('frontend.trackings.destroy', $tracking->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('tracking_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.trackings.massDestroy') }}",
